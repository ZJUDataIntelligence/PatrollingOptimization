...
origin_crime = np.load('90crime10_new.npy')
crime = origin_crime
...
for n_iter in np.arange(180):
    ...
    for ii in np.arange(EP_LEN + 1):
        ...
        for node in action_tuple:
            ...
            beizeng = 1
            for xx in route:
                if end == xx[1]: beizeng = 5
            cost1 = 1
            reward2 = 5000 / action_step
            ...
            if first_crime <= 50:
                ...
                crime[start][end] = first_crime - (first_crime - 50) * 0.1
            reward_cost[end] += cost1 * (parameter) + reward2 * (1 - parameter)
            if rl_end == 0:
                if ii >= 1:
                    ...
                    if start != start_node and number_max > 0 and flag != 3:  # 判断有路线而且过了出发点判定
                        ...
                        route.append(once_action)
                        crime[start_node][next_node] = crime[start][end] * 10
                        action_step = action_step - distance[start_node][next_node]
                        # print "加了一个"
                        flag = 3
                        if next_node == daoda:
    ...
    print "您的距离系数损耗为{},风险系数损耗为{},每步距离损耗{},每步风险损耗{}".format(dj_distance_cost, dj_crime_cost, dj_distance_single_cost,
                                                           dj_crime_single_cost)

    print "A_star  -----  A_star"
    print "第{}天,您的路线为{}".format(n_iter, axing)
    print "您的距离系数损耗为{},风险系数损耗为{},每步距离损耗{},每步风险损耗{}".format(a_distance_cost, a_crime_cost, a_distance_single_cost,
                                                           a_crime_single_cost)
    ...
    one_data = [dj_distance_cost, a_distance_cost, rl_distance_cost, dj_crime_cost, a_crime_cost, rl_crime_cost,
                dj_distance_single_cost, a_distance_single_cost, rl_distance_single_cost, dj_crime_single_cost,
                a_crime_single_cost, rl_crime_single_cost,
...
data = pd.DataFrame(data_save,
                    columns=('dj_distance_cost', 'a_distance_cost', 'rl_distance_cost', 'dj_crime_cost', 'a_crime_cost',
                             'rl_crime_cost',
                             'dj_distance_single_cost', 'a_distance_single_cost', 'rl_distance_single_cost',
                             'dj_crime_single_cost', 'a_crime_single_cost', 'rl_crime_single_cost',
