...
for n_iter in np.arange(180):
    ...
    for ii in np.arange(EP_LEN + 1):
        ...
        for node in action_tuple:
            ...
            # crime[start][end] = random.uniform(0.1, 2) * first_crime
        ...
        if greedy_coverage_end == 0:
            for node in env.nodes:
                if node is not None:  # 有效的node
                    ...
                    if curr_node_id == greedy_now_node:  # 如果剩余的订单不为0
                        ...
                        for neighbor_node in neighbor_list:  # 遍历该节点的邻居节点，
                            ...
                            # print "neighbor_id = {}".format(neighbor_id)
                            # if neighbor_id == daoda and ii < 100:
                                # index = index + 1


                            # print "neighbor_id"
                            # print neighbor_id
                            # print "greedy_now_node"
                            # print greedy_now_node
                            for xx in greedy_coverage:
                                # print "greedy_coverage"
                                # print greedy_coverage
                                if neighbor_id == xx[2] and greedy_now_node == xx[1]:
                                    index = index + 1
                                    route_exist = 1
                                    break
                            ...
                            if route_exist == 0 or (route_exist == 1 and index == number_of_elements):
                                if neighbor_id == 24:
                                    neighbor_id = 10
                                greedy_coverage_single_cost.append(distance[greedy_now_node, neighbor_id])
                                greedy_coverage_cost += distance[greedy_now_node, neighbor_id]
                                greedy_coverage_single_reward.append(crime[greedy_now_node, neighbor_id])
                                greedy_coverage_reward += crime[greedy_now_node, neighbor_id]
                                greedy_coverage_i = greedy_coverage_i + 1
                                once_action = (ii, greedy_now_node, neighbor_id)
                                greedy_coverage.append(once_action)
                                greedy_now_node = neighbor_id
                                route_add = 1
                                if neighbor_id == daoda:
