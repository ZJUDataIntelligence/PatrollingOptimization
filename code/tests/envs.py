...
class CityReal:
    ...
    def __init__(self, mapped_matrix_int, order_num_dist, idle_driver_dist_time, idle_driver_location_mat, order_time_dist, order_price_dist,
        ...
        target_node_ids = []
        target_grids_sorted = np.sort(mapped_matrix_int[np.where(mapped_matrix_int > 0)])#把valid的点顺序排序 [1 2 3 4 5 6 7 8]
    ...
    def construct_mapping(self):
        ...
        :return:
        """
        target_grid_id = self.mapped_matrix_int[np.where(self.mapped_matrix_int>0)]#target_grid_id=[1 3 5 4 2 6 7 8]
    ...
    def utility_add_driver_real_nodewise(self, node_id, num_added_driver):#在每个时刻一开始的作用：num_added_driver是根据输入的mean和std通过正态分布随机取得的值，当值>0是，会用到此函数，为当前node增加idle_driver
    ...
    def utility_bootstrap_oneday_order(self):
        ...
        index_sampled_orders = np.where(np.random.binomial(1, self.p, num_all_orders) == 1)#默认p=1/28.0，但是在example里是1，也就是
    ...
    def step_driver_online_offline_nodewise(self):
        ...
        for idx, target_node_id in enumerate(self.target_node_ids):#target_node_ids好像只有合法点
            ...
            if on_off_number > 0:
                ...
                self.utility_set_drivers_offline_real_nodewise(target_node_id, abs(on_off_number))#on_off_number<0时，让司机随机下线，数量为该值，如果当前空闲driver数量低于该值，则到0，不再下线
            else:
                pass
                ...
                if remain_orders_1d[curr_node_id] != 0:                     #如果剩余的订单不为0
                    for neighbor_node in node.neighbors:                    #遍历该节点的邻居节点，
                        if neighbor_node is not None:                       #邻居节点有效
                            ...
                            remain_orders_1d[curr_node_id] = max(a-b, 0)    #这里为什么不加一个判定if(remain_drivers_1d[neighbor_id]!=0),而是直接计算，这两种代码风格哪一种更好呢
                            remain_drivers_1d[neighbor_id] = max(b-a, 0)    #这里是一个greedy策略，因为他遍历的时候，根据遍历的顺序，在前面的node总是可以优先索取neighbor节点的空闲司机
    ...
    def step(self, dispatch_actions, generate_order=1):
        ...
        reward, reward_node = self.step_assign_order_broadcast_neighbor_reward_update()#reward_node=[node_reward, neighbor_reward]
        ...
        self.step_add_dispatched_drivers(save_remove_id)#这个函数是给调度的司机设计，功能是，在t时刻受调度的司机，在t+1时刻到达了目的地，然后set司机position为目的地node，设置司机状态为online,目的地num_driver+1
