...
origin_distance = np.load('25distance.npy')
origin_crime = np.load('25crime.npy')
distance = origin_distance
crime = origin_crime
hotspots = [[5, 10, 11], [10, 9, 14], [15, 22, 21], [20, 13, 14], [25, 22, 17], [30, 11, 12], [35, 4, 3], [40, 12, 7]]
...
mapped_matrix_int = np.array([[5, 8, 2, 17, 15], [1, 7, 3, 23, 19],
                              [6, 4, 9, 11, 22], [16, 10, 13, -100, 21], [12, 14, 18, 20, 24]])
M, N = mapped_matrix_int.shape
order_num_dist = []
num_valid_grid = 24
...
for ii in np.arange(144):
    ...
    idle_driver_location_mat[ii, :] = [2] * num_valid_grid  # 每个时间点，每个节点上闲置司机的数量

idle_driver_dist_time = [[10, 1] for _ in np.arange(144)]  # 每个时间点，整个模拟器中闲置司机数量的平均值和方差
n_side = 6
l_max = 2
...
for n_iter in np.arange(180):
    distance = origin_distance
    crime = origin_crime
    crime_greedy_coverage = origin_crime
    crime_greedy_reward = origin_crime
    ...
    greedy_coverage_cost = 0
    greedy_coverage_single_cost = []
    greedy_coverage_reward = 0
    greedy_coverage_single_reward = []

    greedy_reward_cost = 0
    greedy_reward_single_cost = []
    greedy_reward_reward = 0
    greedy_reward_single_reward = []
    ...
    axing = []
    dj = []
    greedy_coverage = [once_action]
    greedy_reward = [once_action]

    rl_end_time = 0
    greedy_coverage_end = 0
    greedy_reward_end = 0

    greedy_coverage_step = 1800
    now_greedy_coverage_step = greedy_coverage_step
    greedy_reward_step = 1800
    now_greedy_reward_step = greedy_reward_step

    greedy_coverage_covered = []
    greedy_coverage_now_node = 0
    action_step = 1800  # 总移动步长
    now_step = action_step
    crime_reward = []
    crime_rewards = 0

    greedy_reward_covered = []
    greedy_reward_now_node = 0
    ...
    for ii in np.arange(EP_LEN + 1):
        ...
        #print "day{},{}".format(greedy_reward_i, crime_greedy_reward)
        ...
        for node in action_tuple:
            ...
            if now_step < 0:
                ...
                now_step1 = 1
            reward3 = action_step / now_step1 / find_du(end, daoda, 10)
            ...
            if rl_end == 0:
                if ii >= 1:
                    if start == start_node:  # 判断是否是当前所要出发的点
                        tianjia = 1
                        if end == daoda and now_step > 100:
                            continue
                        if number > number_max and tianjia == 1:
                            number_max = number
                            next_node = end
                            before_node = start
                    if start != start_node and number_max > 0 and flag != 3:  # 已经有了路径选择
                        once_action = (ii, before_node, next_node)
                        route.append(once_action)
                        now_step = now_step - distance[before_node][next_node]
                        flag = 3
                        crime_reward.append([reward2])
                        crime_rewards += reward2
                        if next_node == daoda:
                            now_step1 = now_step
                            if now_step < 0:
                                now_step1 = now_step * (-1)
                            if n_iter >= 50:
                                reward_cost[end] += daoda_parameter1 * (action_step - now_step1) / action_step

                            rl_end = 1
                            rl_end_time = tttt()
        route_coverage_add = 0
        if greedy_coverage_end == 0:
            for node in env.nodes:
                if node is not None:  # 有效的node
                    if route_coverage_add == 1:
                        break
                    curr_node_id = node.get_node_index()  # 当前node的id
                    if curr_node_id == greedy_coverage_now_node:
                        neighbor_list = node.neighbors
                        j = 0
                        for i in range(len(neighbor_list)):
                            if neighbor_list[j] is None:
                                neighbor_list.pop(j)
                            else:
                                j += 1
                        number_of_elements = len(neighbor_list)
                        index = 0

                        for neighbor_node in neighbor_list:  # 遍历该节点的邻居节点，
                            route_exist = 0
                            neighbor_id = neighbor_node.get_node_index()  # 邻居节点的id

                            for xx in greedy_coverage:
                                # print "greedy_coverage"
                                # print greedy_coverage
                                if neighbor_id == xx[2] and greedy_coverage_now_node == xx[1]:
                                    index = index + 1
                                    route_exist = 1
                                    break
                            # print "route_exist = {}".format(route_exist)
                            # print "index = {}".format(index)
                            # print "number = {}".format(number_of_elements)
                            if route_exist == 0 or (route_exist == 1 and index == number_of_elements):
                                #if neighbor_id == 24 or neighbor_id == 0:
                                    #if greedy_coverage_now_node == 1:
                                        #neighbor_id = 6
                                    #if greedy_coverage_now_node == 5:
                                        #neighbor_id = 10
                                if route_exist == 1:
                                    random_patrol_index = random.randint(0, index - 1)
                                    neighbor_id = neighbor_list[random_patrol_index].get_node_index()

                                now_greedy_coverage_step -= distance[greedy_coverage_now_node, neighbor_id]
                                greedy_coverage_single_cost.append(distance[greedy_coverage_now_node, neighbor_id])
                                greedy_coverage_cost += distance[greedy_coverage_now_node, neighbor_id]
                                greedy_coverage_single_reward.append(
                                    crime_greedy_coverage[greedy_coverage_now_node, neighbor_id])
                                greedy_coverage_reward += crime_greedy_coverage[greedy_coverage_now_node, neighbor_id]
                                greedy_coverage_i = greedy_coverage_i + 1
                                once_action = (ii, greedy_coverage_now_node, neighbor_id)
                                greedy_coverage.append(once_action)
                                greedy_coverage_now_node = neighbor_id
                                route_coverage_add = 1
                                if now_greedy_coverage_step <= 50:
                                    greedy_coverage_end = 1
                                break
        route_reward_add = 0
        # print "day{},{}".format(greedy_reward_i, crime_greedy_reward)
        if greedy_reward_end == 0:
            for node in env.nodes:
                if node is not None:  # 有效的node
                    if route_reward_add == 1:
                        break
                    curr_node_id = node.get_node_index()  # 当前node的id
                    if curr_node_id == greedy_reward_now_node:
                        neighbor_list = node.neighbors
                        j = 0
                        for i in range(len(neighbor_list)):
                            if neighbor_list[j] is None:
                        ...
                        for neighbor_node in neighbor_list:  # 遍历该节点的邻居节点，
                            ...
                            now_reward = crime_greedy_reward[greedy_reward_now_node, neighbor_id]
                            if now_reward > best_reward:
                                best_id = neighbor_id
                                best_reward = now_reward
                        #print"now_id = {},best_id = {},best_reward = {}".format(greedy_reward_now_node,best_id,best_reward)
                        now_greedy_reward_step -= distance[greedy_reward_now_node, best_id]
                        greedy_reward_single_cost.append(distance[greedy_reward_now_node, best_id])
                        greedy_reward_cost += distance[greedy_reward_now_node, best_id]
                        greedy_reward_single_reward.append(crime_greedy_reward[greedy_reward_now_node, best_id])
                        greedy_reward_reward += crime_greedy_reward[greedy_reward_now_node, best_id]
                        greedy_reward_i = greedy_reward_i + 1
                        once_action = (ii, greedy_reward_now_node, best_id)
                        greedy_reward.append(once_action)
                        greedy_reward_now_node = best_id
                        route_reward_add = 1

                        if now_greedy_reward_step <= 50:
                            greedy_reward_end = 1
                        break
            ...
            for node in env.nodes:
                if node is not None:
                    ...
                    for neighbor_node in neighbor_list:
                        neighbor_id = neighbor_node.get_node_index()
                        route_exist = 0
                        for xx in greedy_reward:
                            # print "greedy_coverage"
                            # print greedy_coverage
                            if neighbor_id == xx[2] and now_id == xx[1]:
                                for yy in hotspots:
                                    if neighbor_id == yy[2] and now_id == yy[1]:
                                        if xx[0] < yy[0]:
                                            route_exist = 2
                                        else:
                                            route_exist = 1
                                break
                        if route_exist == 1:
                            crime_greedy_reward[now_id, neighbor_id] = random.randint(1, 5)
                        elif route_exist == 2:
                            crime_greedy_reward[now_id, neighbor_id] = random.randint(100, 500)
                        else:
                            crime_greedy_reward[now_id, neighbor_id] = random.randint(10, 50)
        if greedy_reward_end == 0:
            for node in env.nodes:
                if node is not None:
                    now_id = node.get_node_index()
                    ...
                    for neighbor_node in neighbor_list:
                        ...
                        for xx in route:
                            if neighbor_id == xx[2] and now_id == xx[1]:
                                ...
                                break
                        if route_exist == 1:
                            crime[now_id, neighbor_id] = random.randint(1, 5)
                        elif route_exist == 2:
                            crime[now_id, neighbor_id] = random.randint(100, 500)
                        else:
                            crime[now_id, neighbor_id] = random.randint(10, 50)
    ...
    print "greedy_coverage  -----  greedy_coverage"
    print "第{}天,您的路线为{}".format(n_iter, greedy_coverage)
    print "您的距离系数损耗为{},风险系数收益为{},每步距离损耗{},每步收益获得{}".format(greedy_coverage_cost, greedy_coverage_reward,
                                                           greedy_coverage_single_cost,
                                                           greedy_coverage_single_reward)

    print "greedy_reward  -----  greedy_reward"
    print "第{}天,您的路线为{}".format(n_iter, greedy_reward)
    print "您的距离系数损耗为{},风险系数收益为{},每步距离损耗{},每步收益获得{}".format(greedy_reward_cost, greedy_reward_reward,
                                                           greedy_reward_single_cost,
                                                           greedy_reward_single_reward)
    ...
    print "第{}天,crime rewards为{}".format(n_iter, crime_rewards)
    print "第{}天,您的终点为{}".format(n_iter, route[-1][2])
    print "第{}天,您最后所剩step为{}".format(n_iter, now_step)
    print "第{}天,您的路线为{}".format(n_iter, route)
    print "您的距离系数损耗为{},风险系数收益为{},每步距离损耗{},每步风险收益{}".format(rl_distance_cost, rl_crime_cost, rl_distance_single_cost,
                                                           rl_crime_single_cost)
    # print "输入任意数字以确认"
    # suiji = input()

    one_data = [rl_distance_cost, rl_crime_cost, rl_distance_single_cost, rl_crime_single_cost, greedy_coverage_cost, greedy_coverage_reward,
                                                           greedy_coverage_single_cost,
                                                           greedy_coverage_single_reward,greedy_reward_cost, greedy_reward_reward,
                                                           greedy_reward_single_cost,
                                                           greedy_reward_single_reward,route, duration,
                rl_duration]
...
data = pd.DataFrame(data_save,
                    columns=('rl_distance_cost', 'rl_crime_cost', 'rl_distance_single_cost', 'rl_crime_single_cost',
                             'greedy_coverage_cost', 'greedy_coverage_reward', 'greedy_coverage_single_cost',
                             'greedy_coverage_single_reward', 'greedy_reward_cost', 'greedy_reward_reward',
                             'greedy_reward_single_cost', 'greedy_reward_single_reward', 'route', 'oneday_duration',
                             'rl_duration'))

data.to_excel('25_risk10_step80.xls')
