        ...
        if rl_end == 0:
            if len(route) - rl_i == 2:
                ...
                if dj_i < len(dj) - 1:
                    ...
                    dj_i = dj_i + 1
                if greedy_coverage_i < len(greedy_coverage) - 1:
                    greedy_coverage_covered.append(distance(chushi)(mudi))
                    greedy_coverage_cost += distance[chushi][mudi]
                    greedy_coverage_single_cost.append(distance[chushi][mudi])
                    greedy_coverage_reward += crime[chushi][mudi]
                    greedy_coverage_single_reward.append(crime[chushi][mudi])
        ...
        for node in env.nodes:
            if node is not None:  # 有效的node
                ...
                if curr_node_id == greedy_now_node:  # 如果剩余的订单不为0
                    number_of_elements = len(node.neighbors)
                    index = 0
                            ...
                            for xx in greedy_coverage:
                                if neighbor_id == xx[2] and greedy_now_node == xx[1]:
                                    if index < number_of_elements:
                                        ...
                                        greedy_coverage.append(greedy_now_node, neighbor_id)
                                        ...
                                        greedy_coverage_single_reward.append(cost[greedy_now_node, neighbor_id])
                                        greedy_coverage_reward += cost[greedy_now_node, neighbor_id]
                                        greedy_coverage_i = greedy_coverage_i + 1
